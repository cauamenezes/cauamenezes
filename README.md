# Cauã Menezes da Silva

[![LinkedIn](https://img.shields.io/badge/LinkedIn-blue?logo=linkedin&logoColor=white)](https://linkedin.com/in/devcauamenezes) 
[![GitHub](https://img.shields.io/badge/GitHub-black?logo=github&logoColor=white)](https://github.com/cauamenezes) 
[![Email](https://img.shields.io/badge/Email-cauamenezes.07%40gmail.com-red?logo=gmail&logoColor=white)](mailto:cauamenezes.07@gmail.com)

---

# 👨‍💻 Perfil Profissional  

Sou **Tecnólogo em Análise e Desenvolvimento de Sistemas** e **Técnico em Desenvolvimento de Sistemas pelo SENAI**, com mais de **2 anos de experiência em automação de processos (RPA)**, documentação técnica e desenvolvimento web.  

Atuo com o ecossistema **SAP BTP**, incluindo **SAP Build Process Automation**, **SAP Build Apps**, **CAP** e **Hana Cloud**, além de **UiPath**, **Python** e **Power Automate**. Tenho experiência em projetos dos setores **financeiro, logístico e jurídico**, com foco em mapear processos, propor melhorias e entregar soluções automatizadas de alto impacto.  

Entre os projetos de destaque, participei do **Portal de Precificação de Fretes da Votorantim Cimentos**, desenvolvido com **SAP Build Apps, CAP e Hana Cloud**, automatizando cálculos de itinerários e atualização de parâmetros — aumentando a eficiência e confiabilidade do processo.  

Também possuo vivência em **desenvolvimento web full stack**, com foco em front-end, modelagem de dados e bancos relacionais (**SQL**).  

---

## 💼 Experiência Profissional  

**CIDIC | Consultoria em TI**  
*Desenvolvedor RPA e Suporte Técnico*  
📍 *Jan/2023 – Atual | Osasco, SP – Híbrido*  

- 💡 **Cliente Votorantim Cimentos:** desenvolvimento do **Portal de Precificação de Fretes** utilizando **SAP Build Apps, CAP e Hana Cloud** no **SAP BTP**.  
- 🧾 Criação de **PDDs e SDDs**, garantindo documentação técnica clara e rastreável.  
- ⚙️ Desenvolvimento e manutenção de automações em **SAP Build Process Automation, UiPath e Python**, reduzindo custos e otimizando processos.  
- 🧩 Execução de **testes unitários e integrados** junto a Key Users.  
- 🚀 Criação de **POCs** e melhorias contínuas em automações existentes.  
- 👨‍🏫 **Treinamento de equipe** e disseminação de conhecimento sobre SAP BTP.  
- 💻 Desenvolvimento e manutenção de **sistemas web internos**.  
- 🔧 **Suporte N1** a redes e sistemas para clientes como **Solví, Arke Energia e Bombril**.  

---

## 🎓 Formação Acadêmica  

🎓 **Tecnólogo em Análise e Desenvolvimento de Sistemas** – Centro Universitário FIEO *(2023 – 2025)*  
🏫 **Técnico em Desenvolvimento de Sistemas** – SENAI “Prof. Vicente Amato” *(2021 – 2022)*  

---

## 🛠️ Habilidades Técnicas  

| Categoria | Tecnologias |
|------------|--------------|
| **Automação de Processos** | SAP Build Process Automation (SAP BTP), UiPath, Power Automate, Python |
| **Desenvolvimento no SAP BTP** | Build Apps, CAP, Hana Cloud |
| **Documentação Técnica** | PDDs, SDDs, BPMN (AS/IS e TO/BE) |
| **Desenvolvimento Web** | HTML, CSS, JavaScript, Node.js |
| **Banco de Dados** | SQL, Modelagem MER/DER |
| **Metodologias Ágeis** | Scrum, Kanban |
| **Outros** | Java (POO), Análise de dados em Python |

---

## 🌍 Idiomas  

- 🇧🇷 **Português:** Nativo  
- 🇬🇧 **Inglês:** Intermediário (B1)  

---

## 📫 Contato  

📧 **Email:** [seuemail@exemplo.com](mailto:seuemail@exemplo.com)  
💼 **LinkedIn:** [linkedin.com/in/seunome](https://linkedin.com/in/seunome)  
💻 **Portfólio:** [github.com/seuusuario](https://github.com/seuusuario)  

---

⭐ *“Tecnologia é ferramenta, não fim. O valor está no que ela transforma.”*  
